...
export function useAnalytics(jwtToken?: string) {
  ...
  async function executeQuery(params: AnalyticsQueryParams) {
    ...
    try {
      ...
      const response = await $fetch<AnalyticsResponse<any>>(
        `${apiBaseUrl}/analytics/query`,
        fetchOptions
      )
...
export function useChartData() {
  ...
  function formatTimeSeriesData(metrics: AnalyticsMetrics, field: string) {
    // This would typically process time-series data from Analytics Engine
    // For now, return mock time series data based on the timeframe
    ...
    return intervals.map((time, index) => ({
      ...
      value: Math.floor(Math.random() * 100) + (index * 2), // Mock data
      label: time.toLocaleDateString('en-US', range === "30d" ? { month: 'short', day: 'numeric' } : range === "7d" ? { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' } : { hour: '2-digit', minute: '2-digit' })
    }))
  }
