...
import { AiAltTextResponseSchema } from '~/server/utils/schemas'
...
export default defineEventHandler(async (event) => {
  try {
    ...
    if (!imageUrl) {
      createApiError(400, 'Image URL is required in query parameter')
    }
    ...
    try {
      ...
      createApiError(400, 'Invalid URL format')
    }
